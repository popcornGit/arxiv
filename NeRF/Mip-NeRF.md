...
利用一个变量的线性变换的协方差是变量协方差的线性变换这一事实![image](https://user-images.githubusercontent.com/48575896/226819319-b3433a2a-9eb2-4efd-b8bd-6895b3cd7cdc.png)

当圆锥锥体高斯被提升到PE基P后，我们可以识别它的均值和协方差:
...
由于位置编码对每个维度进行独立编码，因此这种编码期望仅依赖于γ(x)的边际分布，并且仅依赖于协方差矩阵的对角线(每个维度方差的向量)。
...
![image](https://user-images.githubusercontent.com/48575896/226836295-bcb58f66-28e8-4701-8e73-7fcc6b504146.png)

如果直接计算这些对角线，那么构建IPE特征的成本与构建PE特征的成本大致相同。

图4加你也·简易地显示了在一维空间中IPE和传统PE特征之间的差异。

![image](https://user-images.githubusercontent.com/48575896/226836755-1b9a8040-9f72-472b-ba08-90bc711d3941.png)

IPE特征是直观的:如果位置编码中的特定频率的周期大于用于构造IPE特征的间隔的宽度，那么在该频率上的编码不受影响。

但是如果周期小于间隔(在这种情况下，间隔上的pe将反复振荡)，那么在该频率上的编码将缩小到零。

简而言之,IPE保留在一段时间内保持不变的频率，并温和地“删除”在一段时间内变化的频率，而PE保留所有频率，up to 手动调优超参数L。

通过以这种方式缩放每个正弦和余弦，IPE特征是有效的抗锯齿位置编码特征，平滑地编码空间体积的大小和形状。

IPE还有效地删除了L作为超参数:它可以简单地设置为一个非常大的值，然后永远不会调优(参见补充)。

### Architecture
除了锥跟踪和IPE特性外，mip-NeRF的行为类似于NeRF，如2.1节所述。

对于每个被渲染的像素，不再是NeRF中的射线，而是投射一个锥体。

我们不是沿着射线对tk采样n个值，而是对tk采样n + 1个值，如前所述，计算跨越每个相邻对采样tk值的间隔的IPE特征。

这些IPE特征作为输入传递到MLP，以产生密度τk和颜色ck，如公式2所示。

![image](https://user-images.githubusercontent.com/48575896/226840660-0e0bdfda-ddfa-4cf7-8dce-988be574b128.png)

mip-NeRF中的渲染遵循公式3。

![image](https://user-images.githubusercontent.com/48575896/226840701-beb3c84e-6d7e-4dbd-8a01-c3951ce0a5bc.png)

回想一下，NeRF使用了具有两个不同mlp的分层抽样过程，一个“粗”和一个“细”。(见公式4)。

![image](https://user-images.githubusercontent.com/48575896/226842110-f39323d7-5122-4090-9e3c-1069b43ed926.png)

这在NeRF中是必要的，因为它PE的特性意味着它的mlp只能学习一个单一比例的场景模型。

但是我们的锥形转换和IPE特征允许我们显式地将尺度编码到我们的输入特征中，从而使MLP能够学习场景的多尺度表示

Mip-NeRF因此使用一个带参数Θ的MLP，我们在分层抽样策略中反复查询。

这有很多好处:模型大小减少了一半，渲染更准确，采样更有效，整体算法变得更简单。

我们的优化问题是:

![image](https://user-images.githubusercontent.com/48575896/226842604-7dc4d1c3-0253-40a8-85a4-9b4fa800622b.png)

因为我们只有一个MLP，“粗”损失必须与“细”损失相平衡，这是通过一个超参数λ来实现的(我们在所有实验中设置λ = 0.1)。

与Mildenhall等人[30]一样，我们的粗样本tc是用分层抽样产生的，而我们的细样本tf是用反变换抽样从alpha compositing weights权重w中抽样的。

与NeRF不同，在NeRF中，精细MLP被给予64个粗样本和128个精细样本的排序并集，在mip-NeRF中，我们简单地从粗模型中采样128个样本，从精细模型中采样128个样本(产生与NeRF中相同数量的总MLP评估，以便公平比较)。

在对tf采样之前，我们稍微修改权重w:

![image](https://user-images.githubusercontent.com/48575896/226846185-7bfcf640-ca5a-4b0e-a92a-44905787ce5c.png)

我们用2-tap max滤镜过滤w，然后用2-tap blur(“blurpool”[51])过滤w，它在w上产生了一个宽而光滑的上包络线。

在将超参数α重新归一化为1之前，将超参数α添加到包膜中，这确保即使在空白区域也能绘制一些样本(我们在所有实验中设置α = 0.01)。

Mip-NeRF是在JaxNeRF [11]， a之上实现的JAX[4]的NeRF的重新实现，实现了更好的精度和训练速度比原来的TensorFlow实现。

我们遵循NeRF的训练程序:Adam[19]的100万次迭代，批大小为4096，学习率为对数退火5·10−4到5·10−6。

有关JaxNeRF和mip-NeRF之间的其他细节和一些其他差异，请参阅补充说明，这些差异不会显著影响性能，并且是我们主要贡献的附带内容:圆锥跟踪、IPE，以及单个多尺度MLP的使用。

## Conclusion
我们提出了mip-NeRF，这是一个多尺度的类似NeRF的模型，解决了NeRF固有的混叠问题。

NeRF的工作原理是投射光线，沿着这些光线编码点的位置，并在不同的尺度上训练单独的神经网络。

相比之下，mip-NeRF投射锥，编码锥形截锥的位置和大小，并训练一个单一的神经网络，在多个尺度上对场景建模。

通过明确地推理抽样和规模，mip-NeRF能够降低相对于NeRF的错误率在我们自己的多尺度数据集上是60%，在NeRF的单尺度数据集上是17%，同时也比NeRF快7%。

Mip-NeRF也能够匹配一个蛮力超采样的NeRF变体的精度，同时是22倍快。

我们希望这里提出的一般技术将对其他致力于提高基于光线跟踪的神经渲染模型性能的研究人员有价值。
