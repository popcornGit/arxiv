...
请注意，像素到空间单元的初始分配是基于相机位置，而不考虑场景几何(因为初始化时不知道)。

一旦NeRF获得了对场景的粗略理解，就可以进一步修剪掉不相关的像素/光线，因为遮挡器的介入，这些像素/光线对特定的NeRF没有贡献。

例如，在图1中，早期的NeRF优化可能推断出单元格F中有一堵墙，这意味着来自图像2的像素可以从单元格a和B中修剪。

我们最初的探索发现，这种额外的能见度削减进一步减少了2倍的trainset。我们在补充文件的A节提供详情。

### Interactive Rendering
我们提出了一种新的交互式渲染方法，以替代第4.3节中Mega-NeRF之上现有快速渲染器的经验评估。

为了满足我们的搜索和救援用例，我们尝试:

(a)保持视觉逼真，

(b)尽量减少训练基础模型之外的任何额外处理时间，

(c)加速渲染，这需要超过2分钟的720p帧与正常光线采样，以更易于管理。

#### Caching
大多数现有的快速NeRF渲染器使用缓存的预计算来加速渲染，这在我们的场景规模上可能并不有效。

例如，Plenoc- tree[45]将不透明度和球面谐波系数的缓存预计算为稀疏体素八叉树。

为我们的场景生成整个8级八叉树需要一个小时的计算，根据亮度格式的不同，需要1到12 GB的内存

添加一个额外的级别将处理时间增加到10小时，八叉树大小增加到55GB，超出了除最大gpu之外的所有gpu的容量。

#### Temporal coherence.
我们探索了一个正交方向，利用时间相干性的交互飞行; 一旦计算出渲染给定视图所需的信息，我们就可以为下一个视图重用其中的大部分信息。

与Plenoctree类似，我们从预计算不透明度和颜色的粗略缓存开始。

与Plenoctree相比，我们在交互式可视化中动态细分树。

图4说明了我们的方法。当相机穿越场景时，我们的渲染器使用缓存的输出来快速生成初始视图，然后执行额外的模型采样轮来进一步细化图像，将这些新值存储到缓存中。

由于每个后续帧都与前一帧有显著的重叠，因此它受益于前一帧的改进，只需要执行少量的增量工作来保持质量。

我们在补编第C节提供进一步详情。

#### Guided sampling.
我们在细化八叉树后进行最后一轮引导射线采样，以进一步提高渲染质量。

我们 通过 使用存储在八叉树结构中的权重的 单次传递 来 渲染光线，而NeRF的传统两阶段分层抽样。

由于我们改进的八叉树为我们提供了一个高质量的场景几何估计，我们只需要在感兴趣的表面附近放置少量的样本。

图5说明了两种方法之间的差异。

与其他快速渲染器类似，我们通过沿着射线积累透光率并在一定阈值后结束采样来进一步加速过程。
![image](https://user-images.githubusercontent.com/48575896/227081892-109381e6-ced5-4fc8-8c79-d48f5fe0bd3e.png)

## Conclusion
我们提出了一种模块化的方法，用于在以前未开发的规模上构建nerf。

我们引入了一种稀疏和感知网络结构，以及一种简单的几何聚类算法，该算法将训练像素划分为不同的NeRF子模块，这些子模块可以并行训练。

这些修改将训练速度提高了3倍以上，同时显著提高了重建质量。

我们对Mega-NeRF之上现有的快速渲染器的经验评估表明，基于交互式nerf的大规模渲染仍然是一个开放的研究问题。

我们主张利用时间平滑来最小化视图之间的冗余计算，这是有价值的第一步。
